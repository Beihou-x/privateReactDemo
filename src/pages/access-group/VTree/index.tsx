...
const VTree = (props, ref) => {
  ...
  const { services, onSelect: onSelectHandle, onAdd, onDelete } = props
  ...
  const handleSearch = async () => {
    try {
      ...
      setDefaultExpandedKeys((treeData && treeData || []).map(item => item.id))
      setLoading(false)
  ...
  const getTreeData = (data) => {
    return data.map(m => {
      if (m.childgroups) {
        return {
          title: <span>{m.name}<span className={styles.color}>({m.device_num || 0})</span></span>,
          key: m.id, children: getTreeData(m.childgroups),
          icon: <div className={styles.position}>
            <PlusOutlined onClick={(e) => addHandle(e, m.id)} />
            <EditOutlined onClick={(e) => editHandle(e, m.id)} />
            <Popconfirm title="是否删除本条数据"
              onCancel={(e: any) => {
                e.stopPropagation();
              }}
              onConfirm={(e: any) => {
  ...
  return (
    <div className={styles.tree}>
      <Spin spinning={loading}>
        <Button style={{ marginBottom: 10 }} size='small' type="primary" onClick={() => onAdd(true)}><PlusCircleOutlined /></Button>
