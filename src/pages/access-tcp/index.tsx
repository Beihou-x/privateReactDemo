...
const TCPManagement = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (val) => (
        <>
          <Link to={`/gather/data/wx_access_tcp_edit/${val.id}`}>编辑</Link>
          <Divider type="vertical" />
          <a onClick={() => handleExpand(val.id)}>详情</a>
          <Divider type="vertical" />
          <a onClick={() =>tagAdd(val, true)}>选择厂商</a>
          <Divider type="vertical" />
          <Link to={`/gather/data/wx_access_tcp_report/${val.id}`}>能力报告</Link>
          <Divider type="vertical" />
          <Link to={`/monitor/${val && val.id}/access_protocol`}>监控</Link>
          <Divider type="vertical" />
          <Link to={`/gather/data/wx_access_tcp_record/${val.id}`}>日志</Link>
        </>
      ),
    },
  ];
  ...
  return (
    <>
      <Card bordered={false}>
        ...
        >
          <Link to="/gather/data/wx_access_tcp_add">新增</Link>
        </Button>
        {/* <DownLoadFile
        ...
        <StandardTable
          ...
          columns={columns}
          services={(params:any) => {
            return protocolSearch({...params, type: "ACCESS"})
          }}
          rowSelection={false}
          tableProps={{
            expandable: {
