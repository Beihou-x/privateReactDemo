...
import { Card, Row, Col, DatePicker, Tooltip } from "antd";
import { checkpointCarUnusual } from "@/services/v2";
...
const Home = (props) => {
  ...
  const showDetail = (isShow, record, type) => {
    const obj = {
      source: record.name,
      start: searchParams.time
        ? searchParams.time.startOf("day").format("X")
        : moment().startOf("day").format("X"),
      end: searchParams.time
        ? searchParams.time.endOf("day").format("X")
  ...
  const getTableData = (val) => {
    ...
    let detect_download = val.reduce((pre, currentVal) => {
      ...
      <Card bordered={false}>
        ...
        <StandardTable
          ...
          services={(params: any) => {
            ...
            return checkpointCarUnusual({ ...params, limit: 99 });
          }}
          rowSelection={false}
