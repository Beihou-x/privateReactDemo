...
import { unitDeviceTotalSearch, bigPicUsableSearch, bigPicUsableHead } from "@/services/v2";
...
const Detect = () => {
  ...
  const getHead = async (id) => {
    const res = await bigPicUsableHead({ id: id })
    setHeadData(res)
  }
  ...
  const columns1 = [
    ...
    // {
    //   title: "不可用数",
    //   dataIndex: "un_available_num",
    //   align: 'center'
    // },
    {
      title: <><Tooltip title="当日接收可用大图数/当日接收大图总数"><QuestionCircleOutlined /></Tooltip> 可用率</>,
        ...
        >
          ...
          <div className={styles.flex}>
            ...
            <div style={{ marginLeft: 30 }}>
              可用率小于90%设备占比：
              <span>{(headData && headData.success_big_quality&&format(headData.success_big_quality)) || 0}%</span>
            </div>
          </div>
          <StandardTable
            rowKey="device_id"
            ...
            services={async (params: any) => {
              ...
              const response = await bigPicUsableSearch({
                ...params,
                id: groupId,
                started,
                ended,
