...
import { Row, Card, Collapse, Input, Form, Tooltip,InputNumber,Select } from "antd";
import { unitDeviceTotalSearch, dataFilterSearch, dataFilterHead } from "@/services/v2";
...
const Detect = () => {
  ...
  const getHead = async (id) => {
    const res = await dataFilterHead({ id: id })
    setHeadData(res)
  }
  ...
  const columns1 = [
    ...
    {
      title: <><Tooltip title="当日接收重复报文数/当日接收报文总数"><QuestionCircleOutlined /></Tooltip> 重复率</>,
      dataIndex: "cfl",
      align: 'center',
        ...
        >
          ...
          <div className={styles.flex}>
            ...
            {/* <div style={{ marginLeft: 30 }}>
              可用率小于90%设备占比：
              <span>{(headData && headData.success_little_quality&&format(headData.success_little_quality)) || 0}%</span>
            </div> */}
          </div>
          <StandardTable
            rowKey="device_id"
            ...
            services={async (params: any) => {
              ...
              const response = await dataFilterSearch({
                ...params,
                id: groupId,
                started,
                ended,
