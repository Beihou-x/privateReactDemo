...
import { unitDeviceTotalSearch, deviceCheckSearch, deviceCheckHead } from "@/services/v2";
...
const Detect = () => {
  ...
  const getHead = async (id) => {
    const res = await deviceCheckHead({ id: id })
    setHeadData(res)
  }
  ...
  const columns1 = [
    ...
    // },
    {
      title: <><Tooltip title="当日接收小图有效数/当日接收小图总数"><QuestionCircleOutlined /></Tooltip> 合格率</>,
    ...
    },
    {
      ...
      render: (val, record) => (
        <>
          <Link to={`/device/wx_access_device/detect/wx_small_pic_qualified_detect_detail/${val && val.device_id}`}>
            <a>查看明细</a>
          </Link>
        </>
      ),
        ...
        >
          ...
          <div className={styles.flex}>
            ...
            <div style={{ marginLeft: 30 }}>
              ...
              <span>
                {(headData &&
                  headData.success_little_quality &&
                  format(headData.success_little_quality)) ||
                  0}
                %
              </span>
            </div>
          ...
          <StandardTable
            ...
            services={async (params: any) => {
              ...
              const response = await deviceCheckSearch({
                ...params,
                id: groupId,
                started,
                ended,
